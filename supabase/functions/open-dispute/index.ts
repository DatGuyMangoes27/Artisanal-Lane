...
Deno.serve(async (request) => {
  try {
    ...
    if (reason.length == 0) {
      return jsonResponse({ error: "A dispute reason is required." }, {
        status: 400,
      });
    }
    ...
    if (order.buyer_id != userId) {
      return jsonResponse({ error: "You cannot dispute this order." }, {
        status: 403,
      });
    }
    ...
    if (sellerId == null) {
      return jsonResponse(
        { error: "This order is missing a seller account." },
        { status: 400 },
      );
    }

    const existingConversation =
      Array.isArray(activeDisputeRow?.dispute_conversations)
    ...
    const workflow = prepareDisputeOpen({
      ...
      existingDispute: activeDisputeRow == null ? null : {
    ...
    if (order.tradesafe_allocation_id) {
      await disputeAllocationDelivery(
        order.tradesafe_allocation_id as string,
        reason,
      );
    }

    let createdDispute = workflow.disputeId == null
      ? null
    ...
    if (workflow.createDispute != null) {
      ...
      if (disputeInsertError != null || data == null) {
        throw disputeInsertError ??
          new Error("Unable to create dispute record.");
      }
      createdDispute = data;
    }
    ...
    if (workflow.createInitialMessage != null) {
